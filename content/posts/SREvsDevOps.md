...
author = ["Hillfolk"]
date = 2022-01-04
lastmod = 2025-01-14T06:53:32+09:00
tags = ["SRE", "DevOPS"]
categories = ["SRE"]
draft = false
weight = 100
...
애자일 환경에서 개발자, 시스템 관리자, 프로그래머는 사일로화되어 동일 제품을 개발하면서도 사용자에게 가치를 제공하는 데 필수적인 정보를 공유하지 않을 수도 있습니다. DevOps 엔지니어는 이러한 복잡성을 줄여, 애플리케이션을 빠르게 변경하는 데 필요한 작업과 안정성을 유지하는 태스크간 격차를 해소합니다.
개발자에게는 빠른 개발을 운영자에게는 안정성 있는 시스템을 제공하는 사람 (조율하는 역할을 하는 사람)


#### 야심차게 도입하면 겪는 문제 {#야심차게-도입하면-겪는-문제}
...
-   SLI(서비스 수준 척도)
    실제 어플리케이션에서 측정한 응답속도, 에러율, 시스템 처리량 등을 말하며 시스템 별로 다를 수 있다.
...
| 주요 업무 | 배포 자동화 지속적인 모니터링 인프라 관리 업무 | 모니터링,지표 수립과 측정 안정성을 위한 출시 관리 |

업무의 차이점

| DevOps | SRE                    |                              |
|--------|------------------------|------------------------------|
| 주요 관심 | 개발 및 배포 과정을 통합 | 확장성, 운영지표, 자동화     |
| 담당자 | 개발에 관심 있는 운영팀 | 운영에 관심 있는 개발팀      |
| 측정 지표 | 배포 빈도 ,배포의 실패율 | 서비스 수준 목표(SLO)의 최소/최대치 (SLO) |
...
### SRE 엔지니어의 신조 {#sre-엔지니어의-신조}


### SLA, SLO 및 SLI {#sla-slo-및-sli}

우리 기업의 특성상 사이트의 신뢰성을 갖추는것이 더욱 중요하다고 생각함 그래서 SER에 대해서 관심을 가지고 필요한 것들을 흡수 하려고 함
동일하게 중요성을 이야기하는 것은 모니터링과 자동화야 이야. SRE는 신뢰성 있는 데이터를 수집해서 그것으로 사이트의 신뢰성을 유지하는것 이고 DevOps는 모니터링을 통해서 소프트웨어의 빠른 개발 주기를 지원하는것임
DevOps 에서는 CD/CI 의 자동화를 중시하고 SRE 에서는 자동화를 통해 표준화된 플랫폼을 구축하는걸 목표료함
